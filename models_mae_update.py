...
class MaskedAutoencoderViT(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3,
                 ...
                 mask_ratio=0.75, interpolate_ratio=0.8):
        super().__init__()
    ...
    def masking(self, sequence, mask_sequence, mask_ratio):
        ...
        ids_shuffle1 = torch.argsort(noise1, dim=1).to(sequence.device)
        ids_shuffle2 = torch.argsort(noise2, dim=1).to(sequence.device)
    ...
    def forward_encoder(self, x, mask, mask_ratio):
        ...
        mask = self.spread_masking.create_masks_batch(mask)
        # add pos embed w/o cls token
        x = x + self.pos_embed[:, 1:, :]

        # masking: length -> length * mask_ratio
        
        x, mask, ids_restore = self.masking(x, mask, mask_ratio)
    ...
    def forward(self, imgs, masks, mask_ratio=0.75):
