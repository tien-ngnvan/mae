...
class MaskingGenerator:
    ...
    def _mask(self, mask, max_mask_patches):
        delta = 0
        # print(f"min_num_patches: {self.min_num_patches}, max_mask_patches: {max_mask_patches}")
...
if __name__ == '__main__':
    ...
    generator = MaskingGenerator(input_size=14, num_masking_patches=40, min_num_patches=4,)
    for i in tqdm(range(1)):
        ...
        if mask.sum() != 40:
            ...
            print(mask)
            print(mask.sum())
    print(f"Last mask: {mask}")
    print(f"Last mask sum: {mask.sum()}")
    
    # convert mask to PIL L image
    gray = np.zeros((14, 14), dtype=np.uint8)
    gray[mask == 1] = 255
    from PIL import Image
    img = Image.fromarray(gray)
    img.save("bitwise_mask_test.png")
